...
public class RecommendationServiceApplicationTests {
	...
	void getRecommendationsByProductId() {
		...
		client.get().uri("/recommendation?productId=" + productId).accept(APPLICATION_JSON).exchange().expectStatus()
	...
	void getRecommendationsMissingParameter() {
		...
		client.get().uri("/recommendation").accept(APPLICATION_JSON).exchange().expectStatus().isEqualTo(BAD_REQUEST)
	...
	void getRecommendationsInvalidParameter() {
		...
		client.get().uri("/recommendation?productId=no-integer").accept(APPLICATION_JSON).exchange().expectStatus()
